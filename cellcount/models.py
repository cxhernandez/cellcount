...
def ConvBNReLUPool(i, o, bn=True, kernel_size=(3, 3), padding=0, p=0.5,
                   pool=False, leaky=True):
...
class FPN(torch.nn.Module):
    def __init__(self, height, width, h=4, ratio=2, d=128):
        ...
        for i in range(1, self.h):
            setattr(self, 'across%s' % i, ConvBNReLUPool(1, self.d, p=0., padding=1, kernel_size=(3, 3)))
        ...
        for i in range(self.h):
            setattr(self, 'conv_2_%s' % i, nn.Sequential(ConvBNReLUPool(self.d, 2 * self.d, p=0., padding=1, kernel_size=(3, 3)),
                                                         ...
                                                         ConvBNReLUPool(2 * self.d, 1, leaky=False, bn=False, p=0., padding=1, kernel_size=(3, 3))
                                                        ))
